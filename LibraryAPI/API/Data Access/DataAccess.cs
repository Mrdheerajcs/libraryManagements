...
namespace API.DataAccess
{
    public class DataAccess : IDataAccess
    {
        ...
        public void CreateNote(NoticeBoard noticeBoard)
        {
            using var connection = new SqlConnection(DbConnection);
            var parameter = new
            {
                note = noticeBoard.NoticeB
            };
            connection.Execute("insert into Notices (noticeB) values (@note);", parameter);
        }
        public bool DeleteNoticeBoard(int NoticeId)
        {
            var deleted = false;
            using (var connection = new SqlConnection(DbConnection))
            {
                var sql = $"delete Notices where Id={NoticeId}";
                deleted = connection.Execute(sql) == 1;
            }
            return deleted;
        }
        public IList<NoticeBoard> GetAllNotice()
        {
            IEnumerable<NoticeBoard> Notices;

            using (var connection = new SqlConnection(DbConnection))
            {
                Notices = connection.Query<NoticeBoard>("select * from Notices;");
            }

            return Notices.ToList();
        }
        public bool UpdateNoticeBoard(NoticeBoard noticeBoard, int id)
        {
            using var connection = new SqlConnection(DbConnection);
            var updeted = false;
            var parameter = new
            ...
            updeted = connection.Execute($"update Notices set NoticeB = @note where id= {id};", parameter)==1;
            return updeted;
        }
        public IList<NoticeBoard> GetCNotes(int id)
        {
            IEnumerable<NoticeBoard> Notices;
            using (var connection = new SqlConnection(DbConnection))
            {
                Notices = connection.Query<NoticeBoard>($"select * from Notices where id= {id};");
            }
            return Notices.ToList();
        }
    }
